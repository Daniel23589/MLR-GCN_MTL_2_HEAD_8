...
#            print(f"##################[DEBUG] image_features.shape AFTER processing: {image_features.shape}")
            ...
            output = 5 * (output * w).sum(-1)            # [B, 20]

            ing_neg = output[:, :self.num_ingredients]   # 10
            ing_pos = output[:, self.num_ingredients:]   # 10

            dish_pos = dish_logits_init                  # [B, 121]
...
#            print("--- después GCN:", tf1.shape, tf2.shape)    # → [B,131,4] Verificar si ambas head pasan por la gcn. Daniel
            ...
            dish_logits       = logits[:, 1,  self.num_ingredients:]     # usamos vía positiva [B,121]
