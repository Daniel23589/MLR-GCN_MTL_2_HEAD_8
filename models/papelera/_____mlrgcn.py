...
class Mlrgcn(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        # --- clases --- Daniel
        self.num_ingredients = 10          # nodos 0-9
        self.num_dishes      = 121         # nodos 10-130
        self.num_total_cls   = self.num_ingredients + self.num_dishes  # 131

        self.prompt_learner = MLCPromptLearner(cfg, classnames, clip_model)
        self.tokenized_prompts = self.prompt_learner.tokenized_prompts
        self.image_encoder = clip_model.visual

        # Cabeza para la tarea de clasificaci�n de platos

        """
        self.fc_dishes = nn.Linear(
            self.image_encoder.attnpool.c_proj.out_features,  # features del encoder visual
            self.num_dishes  # N�mero de clases de platos
        )"
        ...
        # 2) Inicializaci�n para cada Linear del Sequential
        ...
        if cfg.DATASET.NAME == 'mafood':
           ...
           # Imprimir la matriz completa     
            print("\n################1 Matriz de relacion completa:")
            print(self.relation)
            print("#### self.relation.shape", self.relation.shape)

            
            assert self.relation.shape[0] == self.num_total_cls, \
                f"Adjacency size {self.relation.shape} ? {self.num_total_cls}" # Daniel
...
#            print(f"##################[DEBUG] image_features.shape AFTER processing: {image_features.shape}")
            ...
            # Cabeza para la predicci�n de platos
            dish_logits_init = self.fc_dishes(image_features) # Daniel

            # Prompt learner + text encoder
            prompts, tokenized_prompts = self.prompt_learner(cls_id)
            text_features = self.text_encoder(prompts, tokenized_prompts)

            # Normalizacion de los features de imagen y texto
            image_features_norm = image_features / image_features.norm(dim=1, keepdim=True)
            text_features = text_features / text_features.norm(dim=-1, keepdim=True)

            # Aqui si ? añade dimension para conv1d despues de normalizar
...
#            print("#### image_features_norm", image_features_norm.shape)
#            print("#### text_features", text_features[:, :, None].shape)

            # logits ingredients  (Daniel)
            b, c, _ = output.shape                        # c == 262
            # calculamos pesos solo sobre la mitad positiva
            output_half = output[:, c // 2:]              # [B, 131]
            w_half = F.softmax(output_half, dim=-1)       # [B, 131]
            # recreamos el vector de pesos para duplicar sobre toda la salida
            w = torch.cat([w_half, w_half], dim=1)        # [B, 262]
            # aplicamos el re-weighting y sumamos sobre la ultima dimension
            output = 5 * (output * w).sum(-1)             # [B, 262]

            # ahora partimos en NEG y POS de las 131 clases
            half = output.size(1) // 2                    # = 131
            neg_all = output[:, :half]                    # [B, 131]
            pos_all = output[:, half:]                    # [B, 131]

            # extraemos ingredientes y platos de cada mitad
            ing_neg  = neg_all[:, :self.num_ingredients]       # [B, 10]
            dish_neg = neg_all[:, self.num_ingredients:]       # [B, 121]
            ing_pos  = pos_all[:, :self.num_ingredients]       # [B, 10]
            dish_pos = pos_all[:, self.num_ingredients:]       # [B, 121]

            # concatenamos para tener 131 nodos NEG y 131 nodos POS
            combined_neg = torch.cat([ing_neg,  dish_neg], dim=1)  # [B, 131]
            combined_pos = torch.cat([ing_pos,  dish_pos], dim=1)  # [B, 131]

            # finalmente, la identidad para la GCN: 262 nodos (NEG+POS)
            identity = torch.cat([combined_neg, combined_pos], dim=1)  # [B, 262]


#            print("--- antes GCN:", combined_neg.shape, combined_pos.shape)   # ? [B,131] Verificar si ambas head pasan por la gcn. Daniel
            
            # GCN por cada mitad (Daniel)
            tf1 = self.gc1(combined_neg, self.gcn_relation.cuda())
            tf1 = self.relu(tf1)
            tf1 = self.gc2(tf1, self.gcn_relation.cuda())
            tf1 = self.relu2(tf1)
            tf1 = self.gc3(tf1, self.gcn_relation.cuda())

            tf2 = self.gc1(combined_pos, self.gcn_relation.cuda())
            tf2 = self.relu(tf2)
            tf2 = self.gc2(tf2, self.gcn_relation.cuda())
            tf2 = self.relu2(tf2)
            tf2 = self.gc3(tf2, self.gcn_relation.cuda())

#            print("--- despu�s GCN:", tf1.shape, tf2.shape)    # ? [B,131,4] Verificar si ambas head pasan por la gcn. Daniel

            output = torch.cat((tf1, tf2), dim=1).squeeze(-1)  # [B,262]
            output += identity

            logits = output.view(b, 2, self.num_total_cls)      # [B,2,131]

            # separar de nuevo (Daniel)
            ingredient_logits = logits[:, :, :self.num_ingredients]      # [B,2,10]
            dish_logits       = logits[:, 1,  self.num_ingredients:]     # usamos via positiva [B,121]

#            print("##### ingredient_logits: ", ingredient_logits.shape)
#            print("##### dish_logits: ", dish_logits.shape)

            return ingredient_logits, dish_logits
