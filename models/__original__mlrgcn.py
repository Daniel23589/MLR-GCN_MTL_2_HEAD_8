...
class Mlrgcn(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        # --- clases --- Daniel
        self.num_ingredients = 18         # nodos 0-17
        self.num_dishes      = 172         # nodos 18-172
        self.num_total_cls   = self.num_ingredients + self.num_dishes  # 190
...
#            print("#### text_features", text_features[:, :, None].shape)
            ...
            output = 5 * (output * w).sum(-1)            # [B, 36]

            ing_neg = output[:, :self.num_ingredients]   # 18
            ing_pos = output[:, self.num_ingredients:]   # 18

            dish_pos = dish_logits_init                  # [B, 172]
            dish_neg = torch.zeros_like(dish_pos)        # sin concepto «negativo»

            # CONCAT 131 nodes (Daniel)
            combined_neg = torch.cat([ing_neg, dish_neg],  dim=1)   # [B,190]
            combined_pos = torch.cat([ing_pos, dish_pos],  dim=1)   # [B,190]

            identity = torch.cat([combined_neg, combined_pos], dim=1)  # [B,380]
...
#            print("--- después GCN:", tf1.shape, tf2.shape)    # → [B,190,4] Verificar si ambas head pasan por la gcn. Daniel

            output = torch.cat((tf1, tf2), dim=1).squeeze(-1)  # [B,380]
            output += identity

            logits = output.view(b, 2, self.num_total_cls)      # [B,2,190]

            # separar de nuevo (Daniel)
            ingredient_logits = logits[:, :, :self.num_ingredients]      # [B,2,18]
            dish_logits       = logits[:, 1,  self.num_ingredients:]     # usamos vía positiva [B,172]
