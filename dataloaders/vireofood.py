...
class vireofood(data.Dataset):
    def __init__(self, root, data_split, img_size=224, p=1, annFile="", label_mask=None, partial=1+1e-6):
        # data_split = train / val / test
        self.root = root
        self.classnames = ['Beverages', 'CerealsAndGrainProducts', 'CulinaryErbs', 'Dumpling', 'Eggs', 'FatsAndOils', 'FishAndSeafood', 'Friedfood', 'Fruits', 'Meat', 
                           'MilkAndDairyProducts', 'Mushrooms', 'NutSeedAndOliveProducts', 'Sauces', 'SausagesAndColdCuts', 'Spices', 'SugarAndSweets', 'VegetablesAndLegumes']
        ...
        dataset_base_path = '/media/HDD_4TB_2/daniel/gnn_test/3.MLR-GCN/MLR-GCN_MTL_VIREOFOOD_2_HEAD_6/food_datasets/vireofood/'

        # Carga de etiquetas multi-label (ingredientes)
        self.labels_lab = np.load(os.path.join(dataset_base_path, 'vireofood_labels.npy'), allow_pickle=True).item()
        ...
        train_transform = transforms.Compose([
            ...
            CutoutPIL(cutout_factor=0.1), # Cutout menos agresivo (factor=0.1 en lugar de 0.5)
#            RandAugment(),
            transforms.ToTensor(),
        ...
        if self.data_split == 'train':
            ...
            raise ValueError(f"data split = {self.data_split} is not supported in vireofood")
    ...
    def __getitem__(self, index):
        # Ruta de la imagen
        img_path = os.path.join('/media/HDD_4TB_2/daniel/gnn_test/3.MLR-GCN/MLR-GCN_5/food_datasets/vireofood/ready_chinese_food/', self.image_list[index])
