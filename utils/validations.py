...
def validate(data_loader, model, args):
    ...
    preds = []
    targets = []
    dish_preds_list = [] # Daniel
    dish_targets_list = [] # Daniel
    ...
    with torch.no_grad():
        ...
        for i, (images, target_ingredients, target_dishes) in enumerate(data_loader):
            ...
            # Guardar platos
            dish_preds_list.append(dish_preds.cpu()) # Daniel
            dish_targets_list.append(target_dishes.cpu()) # Daniel
    ...
    # === Guardar archivos === Daniel
#    np.save('ingredient_preds.npy', torch.cat(preds).numpy())
#    np.save('ingredient_targets.npy', torch.cat(targets).numpy())
#    np.save('dish_preds.npy', torch.cat(dish_preds_list).numpy())
#    np.save('dish_targets.npy', torch.cat(dish_targets_list).numpy())        
    # ========================
