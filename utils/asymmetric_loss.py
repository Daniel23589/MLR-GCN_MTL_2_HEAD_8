...
class AsymmetricLoss_imbalanced(nn.Module):
    ...
    def forward(self, x, y):
        ...
        # recortar solo a las clases presentes en x (ingredientes) Daniel
        n_cls = x.shape[2]                    # p.ej. 10
        weighting_vector = (self.unblanced_vector[:n_cls]    # << recorte crítico
                            .unsqueeze(0)
                            .repeat(x.shape[0], 1)
                            .reshape(-1))
        weighting_vector = weighting_vector / self.reweigh_n

        # (mantener) refuerzo para clases sin peso
        weighting_vector[weighting_vector == 0] += 15 * weighting_vector.max()

        # (opcional) seguridad
        assert weighting_vector.numel() == x.shape[0] * n_cls,\
            "weighting_vector y logits no coinciden en longitud"
